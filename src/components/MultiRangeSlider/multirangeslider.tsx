...
const MultiRangeSlider = (props: Props, ref: React.ForwardedRef<HTMLDivElement>): JSX.Element => {
	...
	const onLeaveThumb = () => {
		...
		const triggerChange = () => {
			...
			isChange && props.onChange && props.onChange(result);
			//alert(result)
			props.onMouseUp && props.onMouseUp(result);
		};
		setMinCaption(props.minCaption || minValue.toFixed(fixed));
		setMaxCaption(props.maxCaption || maxValue.toFixed(fixed));
		let _barMin = ((minValue - min) / (max - min)) * 100;
	...
	var maxLimit = props.maxLimit ? props.maxLimit : 100
	var minCap = (maxLimit / 100) * Number(minCaption)
	var maxCap = (maxLimit / 100) * Number(maxCaption)
	...
	return (
		<div ref={ref} className={(props.baseClassName || 'multi-range-slider') + ' ' + (props.className || '')} style={props.style} onWheel={onMouseWheel}>
			<div className='bar' ref={refBar}>
				...
				<div className='thumb thumb-left' style={{ backgroundColor: props.thumbLeftColor }} onMouseUp={onLeaveThumb} onMouseDown={onLeftThumbMousedown} onTouchStart={onLeftThumbTouchStart}>
					<div className='caption'>
						<span className='min-caption'>{minCap}</span>
					</div>
				</div>
				
				<div className='bar-inner' style={{ backgroundColor: props.barInnerColor, border: 'none' }}>
				...
				<input className='input-type-range input-type-range-max' type='range' min={min} max={max} step={step} value={maxValue} onMouseUp={() => { alert('up') }} onInput={onInputMaxChange} />
				<div className='thumb thumb-right' style={{ backgroundColor: props.thumbRightColor }} onMouseUp={onLeaveThumb} onMouseDown={onRightThumbMousedown} onTouchStart={onRightThumbTouchStart}>
					<div className='caption'>
						<span className='max-caption'>{maxCap}</span>
					</div>
				</div>
